...
torch.save(nc9model.state_dict(), Path(MODEL_PATH, 'best_NC9'))
...
torch.save(nc9mmodel.state_dict(), Path(MODEL_PATH, 'best_NC9M'))
...
torch.save(nc10model.state_dict(), Path(MODEL_PATH, 'best_NC10'))
...
torch.save(nc10mmodel.state_dict(), Path(MODEL_PATH, 'best_NC10M'))
...
torch.save(nc11model.state_dict(), Path(MODEL_PATH, 'best_NC11'))
...
torch.save(nc11mmodel.state_dict(), Path(MODEL_PATH, 'best_NC11M'))
...
torch.save(nc12model.state_dict(), Path(MODEL_PATH, 'best_NC12'))
...
torch.save(nc12mmodel.state_dict(), Path(MODEL_PATH, 'best_NC12M'))
...
torch.save(nc13model.state_dict(), Path(MODEL_PATH, 'best_NC13'))
results.append(('NC13', len(nc13_classifier), nc13_classifier.best_val_loss))

print('\nNC13M Classifier:')
nc13m_classifier = IndividualClassifier('NC13M', 'NC13+', DATA_PATH, DEVICE)
...
torch.save(nc13mmodel.state_dict(), Path(MODEL_PATH, 'best_NC13M'))
